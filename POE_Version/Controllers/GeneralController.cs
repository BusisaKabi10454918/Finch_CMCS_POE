...
namespace PROG.Controllers
{
    public class GeneralController : Controller
    {
        ...
        public async Task<IActionResult> Login(string username, string password)
        {
            ...
            //Not a manager, check HR administrators
            var humanResourcesCheck = await _context.HRAdministrators
                .FirstOrDefaultAsync(h => h.Username == username);

            if (humanResourcesCheck != null && BCrypt.Net.BCrypt.Verify(password, humanResourcesCheck.PasswordHash))
            {
                // Store the logged-in administrator’s ID in session
                HttpContext.Session.SetString("HumanResourceId", humanResourcesCheck.HumanResourcesID.ToString());
                return RedirectToAction("HumanResourcesDash", "HumanResources");
            }
        ...
        public IActionResult Register(string firstName, string lastName, string username, string password, string role)
        {
            ...
            {
                var HRadmin = new Models.HumanResources
                {
                    FirstName = firstName,
                    LastName = lastName,
                    Username = username,
                ...
                _context.HRAdministrators.Add(HRadmin);
                _context.SaveChanges();

                return RedirectToAction("Login View", "General");
            }
